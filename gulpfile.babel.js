...
task(
  'authoring',
  series('css', () => {
    let readme = fs.readFileSync('./pages/authoring.md', {
      encoding: 'utf8',
    });
    readme = replaceMd(readme);
    const article = articleData(readme, 'D MMMM YYYY', 'en'); // TODO change to 'ru' after moment/moment#2634 will be published
...
task(
  'instruction',
  series('css', () => {
    let readme = fs.readFileSync('./pages/instruction.md', {
      encoding: 'utf8',
    });
    readme = replaceMd(readme);
    const article = articleData(readme, 'D MMMM YYYY', 'en'); // TODO change to 'ru' after moment/moment#2634 will be published
...
task('banners', () => src('dump/images/*-banner*').pipe(dest('dist/images')));
...
task('static', () =>
  src(['static/**', 'static/.**', 'node_modules/bootstrap/dist/**']).pipe(
    dest('dist')
  )
);
